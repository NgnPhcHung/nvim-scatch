local macos_path = "/Users/nguyenphuchung/Library/Mobile Documents/iCloud~md~obsidian/Documents/PhucHungVaults"
local path = vim.fn.expand('%:p')

local function git_auto_sync()

  local vault_path = macos_path
  print(path:find(vault_path))
  if not path:find(vault_path, 1, true) then
      print("File is not in the vault: " .. path)
      return 
  end
  ...
  vim.fn.jobstart(add_cmd, {
    on_exit = function(job_id_add, data_add, event_add)
      ...
      if data_add == 0 then -- git add successful
        ...
        local commit_cmd = 'git -C ' .. vim.fn.shellescape(vault_path) .. ' commit -m ' .. vim.fn.shellescape(commit_message)
        ...
        vim.fn.jobstart(commit_cmd, {
          on_exit = function(job_id_commit, data_commit, event_commit)
            ...
            if data_commit == 0 then -- git commit successful
              ...
              vim.fn.jobstart(push_cmd, {
                on_exit = function(job_id_push, data_push, event_push)
                  ...
                  if data_push == 0 then
                    ...
                    vim.notify("Git push failed (Code: " .. data_push .. "). Check terminal for errors.", vim.log.levels.ERROR, { title = "Git Sync" })
                  end
                end
              })
            else
              ...
              vim.notify("Git commit skipped or failed (Code: " .. data_commit .. "). No changes or conflict.", vim.log.levels.WARN, { title = "Git Sync" })
            end
          end
        })
      else
        vim.notify("Git add failed (Code: " .. data_add .. "). Check terminal for errors.", vim.log.levels.ERROR, { title = "Git Sync" })
      end
    end
  })
end
